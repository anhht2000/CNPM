...
//		response.addHeader(REFRESH_TOKEN, uuid.toString());
	...
	public static Authentication parseTokenToUserInformation(HttpServletRequest request, IUserService service) {
		...
		try {
			// parse the token
			String email = Jwts.parser().setSigningKey(SECRET).parseClaimsJws(token.replace(PREFIX_TOKEN, ""))
					.getBody().getSubject();
			return email != null
					? new UsernamePasswordAuthenticationToken(email, null,
							AuthorityUtils
									.createAuthorityList(service.getUserByEmail(email).getRole().getCode()))
					: null;
